...
**KH: My sample has an even lower proportion of people who believe science does not benefit them, 16%*
```{r}
samp1 |> 
  count(scientist_work) |> 
  mutate(sp = n/sum(n))
...
**KH: No, I would expect sample proportions to be different as the samples themselves would be different. I would expect them to be somewhat different as they are still pulled from the same population.**
...
**KH: In this new sample, the Doesn't Benefit proportion is higher, but still close to the population percentage. Additional, larger samples, should be closer to the population proportion, and would provide a more accurate estimae of hte population proportion.**
```{r}
...
**There are 15,000 observations in the sample_props50 dataframe, representing the samples, the number of "Doesn't benefit" observed in each sample, and the sample proportion of "Doesn't benefit" for the sample.**

**The distribution is similar to a normal distribution, with a center around 0.2**
```{r}
...
**KH: There are 25 observations in the smaller sampling distribution. Each observation represents a sample size of 10 and the proportion of "Doesn't benefit" found in each sample, or observation.**
```{r}
...
**Each observation represents the proportion of "Doesn't benefit" found in each sample. The mean, standard error, and shape come closer to a normal distribution and the population proportions as the sample size increases.Increasing simulation size does not show significant change past 10 simulations (using sample size 100).**
...
**The proportion for my sample of "Benefits" is 60%. My best point estimate with the sample I have can only be 60%, as I have no other information on the population (such as other or larger samples)**
```{r}
...
**From this sampling distribution, I would estimate that the true proportion of those who think scientists enhance their lives to be approximately 80%. The shape roughly follows the normal distribution, but it can be argued that it has some skew.**
```{r}
...
**This distribution looks far closer to a normal distribution than the 15 sample size. I would estimate the true proportion of benefit to be approximately 80%, but with much more confidence than the 15 sample size.**
```{r}
...
**The larger sample size has the smaller spread compared to the 15 sample size distribution. To make estimates closer to the true value, we should prefer a sampling distribution with a small spread.**
